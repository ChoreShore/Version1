...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (contract.employer_id !== user.id) {
      throw createError({ statusCode: 403, statusMessage: 'Only the employer can approve the completion' });
    }

    if (contract.status !== 'pending_review') {
      throw createError({
        statusCode: 400,
        statusMessage: `Cannot approve a contract with status "${contract.status}". Only contracts pending review can be approved.`
      });
    }
    ...
    if (updateError || !updated) {
      throw createError({ statusCode: 500, statusMessage: 'Failed to approve contract completion' });
    }
    ...
    if (jobTitle) {
      sendNotificationEmail(event, {
        ...
        subject: `Work approved for "${jobTitle}"`,
        html: `<p>Hi there,</p><p>Great news — the employer has approved your work on "<strong>${jobTitle}</strong>". The payment will be released to you shortly.</p><p>Log in to your dashboard to view the payment status.</p>`,
        idempotencyKey: `contract-approved/${contractId}`
      }).catch(() => {});
    }
    ...
    throw createError({
      ...
      statusMessage: error.statusMessage || 'Failed to approve contract completion'
    });
  }
});
