...
describe('Payments Disconnect Routes', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockServerSupabaseUser.mockResolvedValue({ id: 'user-1', email: 'test@example.com' });
  });

  describe('POST /api/payments/methods/disconnect', () => {
    let handler: any;

    beforeEach(async () => {
      const { default: imported } = await import('~/server/api/payments/methods/disconnect.post');
      handler = imported;
    });

    it('disconnects payment method successfully', async () => {
      const method = {
      ...
      expect(result.method.connection_status).toBe('disconnected');
    });
