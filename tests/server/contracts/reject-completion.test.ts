...
describe('POST /api/contracts/[id]/reject-completion', () => {
  let handler: any;

  beforeEach(async () => {
    vi.clearAllMocks();
    mockServerSupabaseUser.mockResolvedValue({ id: 'employer-1', email: 'employer@example.com' });
    const { default: imported } = await import('~/server/api/contracts/[id]/reject-completion.post');
    handler = imported;
  });

  it('rejects pending_review contract and reverts to active when employer requests', async () => {
    const contract = {
      id: contractId,
  ...
  it('throws 403 when worker tries to reject', async () => {
